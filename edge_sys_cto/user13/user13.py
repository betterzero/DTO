...
device_user_node_topic = "usernode13/request/area1"
...
try:
    while Run_Flag:
        ...
        if client.connected_flag:
            # print("user device "+ str(13) + " is publishing, target to controller")
            client.publish(pub_topic, generate_workload.user_node_workload())
            time.sleep(0.5)
            if len(usable_server_info) != 0:
                # print("user device "+ str(13) + " is publishing, target to edge server")
                client.publish("usernode13/request/area1/edge", str(request_server_resource_amount))
                time.sleep(0.5)
        time.sleep(1)
