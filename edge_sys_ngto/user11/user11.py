...
device_user_node = "usernode11"
# One usernode device has several edge server sub_topics and the quantity equal to the number of edge servers
user_node_clients = [
{"broker":"192.168.0.103","port":1883,"name":"blank","sub_topic":"usernode9","pub_topic":device_user_node}
]
...
# related user
related_user_workload = {"usernode1": 300, "usernode3": 2400, "usernode5": 2400, "usernode7": 500, "usernode9": 2400, \
                        "usernode11": 2400, "usernode13": 300, "usernode15": 2400, "usernode17": 300}
...
related_edge_user_pair = {"edgeserver2": ["usernode1", "usernode3", "usernode5", "usernode7", "usernode9", "usernode11"], \
                          ...
                          "edgeserver6": ["usernode7", "usernode9", "usernode11","usernode15", "usernode17"]}

# resource requirement and cost(designed to be scalable)
usable_server_info = {"edgeserver2": 11600, "edgeserver4": 6600, "edgeserver6": 15100}
...
class CommunicateDevice:
    ...
    def on_message(self, client, userdata, message):
        ...
        if len(update_reachable_device_topic_unique) == quantity_user_node_clients and ndto_flag:
...
class StrategicDecision:
    ...
    def probability_init(self):
        global probability_to_server
        user_node_dic = {}

        for user_node in related_user_workload.keys():
            probability_to_server[user_node] = {}

        for server in related_edge_user_pair.keys():
            for user_node in related_edge_user_pair[server]:
                if user_node in user_node_dic:
                    user_node_dic[user_node] += 1
                else:
                    user_node_dic[user_node] = 1

        for server in related_edge_user_pair.keys():
            for user_node in related_edge_user_pair[server]:
                    probability_to_server[user_node][server] = 1 / user_node_dic[user_node]

    def delay_error_init(self):
        global delay_error
        for user_node in related_user_workload.keys():
            delay_error[user_node] = 0
    ...
    def ndto_strategy(self):
        ...
        threshold_value = 0.0000000000000001
        ...
        probability_device = self.best_reply(server_remain) # this
        ...
        delay_pre = delay_cur
        norm_value = sum(list(delay_error.values()))
...
strategic_decision_init = StrategicDecision()
strategic_decision_init.probability_init()
strategic_decision_init.delay_error_init()
...
try:
    while Run_Flag:
        ...
        if client.connected_flag:
            #print("user device "+ str(11) + " is publishing~~~~~~~~~~")
