...
device_user_node_topic = "usernode11/request/area1"
...
{"broker":"192.168.1.111","port":1883,"name":"blank","sub_topic":"edgeserver6/available/area1","pub_topic":device_user_node_topic}
]
...
class StrategicDecision:
    ...
    def user_node_measure(self):
        ...
        probability_server6 = Variable()
        constraints = [probability_server2 >= 0, probability_server2 <= 1, probability_server4 >=0, probability_server4 <=1, probability_server6 >= 0, probability_server6 <= 1]

        probability_total = 0
        probability_total = probability_server2 + probability_server4 + probability_server6
        ...
        for server in usable_server_info.keys():
            ...
            if server == "edgeserver4/available/area1":
                ...
                temp2_total += probability_to_server[server] / ( usable_server_info[server][0] * ( 1 - usable_server_info[server][1] ) ** 2 )
            if server == "edgeserver6/available/area1":
                temp1_total += probability_server6 / ( usable_server_info[server][0] * ( 1 - usable_server_info[server][1] ) ** 2 )
                temp2_total += probability_to_server[server] / ( usable_server_info[server][0] * ( 1 - usable_server_info[server][1] ) ** 2 )
        ...
        for server in usable_server_info.keys():
            ...
            if server == "edgeserver6/available/area1":
                temp = usable_server_info[server][0] * ( 1 - usable_server_info[server][1] ) / ( generate_workload.user_node_workload() ) + probability_to_server[server]
                average_computing_delay +=  temp * ( temp - probability_server6 ) ** (-1)
        ...
        for server in usable_server_info.keys():
            ...
            if server == "edgeserver6/available/area1":
                server_probability[server] = probability_server6.value
