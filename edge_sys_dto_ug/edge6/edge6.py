...
device_edge_server_topic = "edgeserver6/available/area1"
...
{"broker":"192.168.1.111","port":1883,"name":"blank","sub_topic":"usernode15/request/area1","pub_topic":device_edge_server_topic},
{"broker":"192.168.1.111","port":1883,"name":"blank","sub_topic":"usernode17/request/area1","pub_topic":device_edge_server_topic}
]

quantity_edge_server_clients = len(edge_server_clients)

update_reachable_device_topic_unique = []    # online device and the value is unique
...
request_users_info = {}                      # the message this device subscribe, (maybe) need to performe a sum operation to dict_values()
result_server_sent = {}                      # the result this server to send, for dto_ug-algorithm, is p(utilization rate) and computation resource capacity
pub_flag = False
computation_resource_capacity = 15100        # the total computation capacity of this server
...
class CommunicateDevice:
    ...
    def on_message(self, client, userdata, message):
        ...
        if update_reachable_device_topic_unique.count(user_topic) == 0:
            ...
            # store the subscribed client(device) key-value
            user_content = str(message.payload.decode("utf-8"))
            user_content_dict = eval(user_content)
            user_content_digital = user_content_dict[device_edge_server_topic]
        ...
        if len(update_reachable_device_topic_unique) == quantity_edge_server_clients and round_count < iteration_round:
            ...
            strategic_decision.dto_ug_obtain_data_to_sent()
            round_count += 1
            pub_flag = True
...
class StrategicDecision:
    ...
    def dto_ug_obtain_utilization_rate(self):
    ...
    def dto_ug_obtain_data_to_sent(self):
        global result_server_sent
        utilization_rate = self.dto_ug_obtain_utilization_rate()
...
try:
    while Run_Flag:
        ...
        if client.connected_flag and pub_flag:
            #print("edge server "+ str(6) + " is publishing~~~~~~~~~~" + pub_topic)
