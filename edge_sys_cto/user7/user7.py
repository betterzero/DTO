...
try:
    while Run_Flag:
        ...
        if client.connected_flag:
            # print("user device "+ str(7) + " is publishing, target to controllers")
            client.publish(pub_topic, generate_workload.user_node_workload())
            time.sleep(0.5)
            if len(usable_server_info) != 0:
                # print("user device "+ str(7) + " is publishing, target to edge server")
                client.publish("usernode7/request/area1/edge", str(request_server_resource_amount))
                time.sleep(0.5)
        time.sleep(1)
